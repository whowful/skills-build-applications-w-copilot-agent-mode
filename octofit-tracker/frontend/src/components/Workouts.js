...
const WORKOUTS_ENDPOINT = process.env.REACT_APP_CODESPACE_NAME
...
const getItemLabel = (item) => item.name || item.title || item.username || 'Unnamed workout';
...
function Workouts() {
  ...
  const endpoint = WORKOUTS_ENDPOINT;
      ...
      <div className="card-body">
        ...
        <form className="row g-2 align-items-end mb-3" onSubmit={(event) => event.preventDefault()}>
          <div className="col-md-8">
            <label className="form-label fw-semibold" htmlFor="workouts-search">Search workouts</label>
            <input
              id="workouts-search"
